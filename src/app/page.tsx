...
import {
  ...
  BarChart2, X, Plus, Trash2, Save, MoreVertical, Brain, GripVertical,
  Settings2, EyeOff, History, Map as MapIcon, SlidersHorizontal, HelpCircle,
} from 'lucide-react';

// Types
import { type RsiFilter, type MacdFilter, type StochFilter, type ActiveTab, MASTER_ID } from '@/types';
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans pb-10">
      ...
      <header className="bg-slate-800 border-b border-slate-700 py-4 px-6 sticky top-0 z-10 flex justify-between items-center shadow-md">
        ...
        <div className="flex items-center gap-1">
          <div className="flex rounded-lg overflow-hidden border border-slate-700 text-sm">
            {([
              { id: 'watchlist', label: 'Watchlist', icon: <BarChart2 size={14}/>, color: 'blue' },
              { id: 'history',   label: 'Lịch Sử',   icon: <History size={14}/>,  color: 'violet', badge: sd.signalHistory.length },
              { id: 'heatmap',   label: 'Heatmap',   icon: <MapIcon size={14}/>,  color: 'emerald' },
              { id: 'screener',  label: 'Screener',  icon: <SlidersHorizontal size={14}/>, color: 'amber' },
            ] as { id: ActiveTab; label: string; icon: React.ReactNode; color: string; badge?: number }[]).map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-2 px-4 py-2 transition-colors ${
                  activeTab === tab.id
                    ? `bg-${tab.color}-500/20 text-${tab.color}-300 font-semibold`
        ...
        {activeTab === 'watchlist' && <>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <select value={rsiFilter} onChange={e => setRsiFilter(e.target.value as RsiFilter)} className="bg-slate-900 border border-slate-700 rounded-md px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500">
                <option value="ALL">RSI: All</option><option value="OVERSOLD">RSI &lt; 30</option>
                <option value="NEUTRAL">RSI 30–70</option><option value="OVERBOUGHT">RSI &gt; 70</option>
              </select>
              <select value={macdFilter} onChange={e => setMacdFilter(e.target.value as MacdFilter)} className="bg-slate-900 border border-slate-700 rounded-md px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500">
                <option value="ALL">MACD: All</option><option value="BULLISH">Bullish</option><option value="BEARISH">Bearish</option>
              </select>
              <select value={stochFilter} onChange={e => setStochFilter(e.target.value as StochFilter)} className="bg-slate-900 border border-slate-700 rounded-md px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500">
                <option value="ALL">Stoch: All</option><option value="OVERSOLD">Oversold &lt; 20</option>
                <option value="OVERBOUGHT">Overbought &gt; 80</option><option value="BULLISH_CROSS">Bullish Cross</option><option value="BEARISH_CROSS">Bearish Cross</option>
              </select>
            </div>
                        ...
                        return (
                          <div key={ticker} onClick={() => handleSignalTickerClick(ticker)}
                            className={`relative flex flex-col px-2.5 py-1.5 border rounded-lg text-xs cursor-pointer hover:brightness-110 transition-all group ${
                              score === 3 ? `bg-${c}-500/25 border-${c}-500/50 ring-1 ring-${c}-500/30` :
                              score === 2 ? `bg-${c}-500/15 border-${c}-500/35` : `bg-${c}-500/10 border-${c}-500/20`
                            }`}>
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-[9px] px-1.5 py-0.5 rounded font-mono leading-none ${
                                score === 3 ? `bg-${c}-400/20 text-${c}-300 font-bold` :
                                score === 2 ? `bg-${c}-500/15 text-${c}-400` : `bg-slate-700 text-${c}-600`
                              }`}>{score}/3</span>
                              <button onClick={e => { e.stopPropagation(); setIgnoredSignalTickers(p => [...p, ticker]); }}
                  ...
                  <tbody className="divide-y divide-slate-700/50">
                    {sd.loading ? (
                      ...
                      <tr><td colSpan={14} className="px-6 py-12 text-center text-slate-500">
                        {wl.activeWatchlist && wl.activeWatchlist.tickers.length === 0
                      ...
                      return (
                        <Fragment key={item.ticker}>
                          <tr id={`row-${item.ticker}`}
                            draggable onDragStart={() => setTableDragIdx(rowIdx)}
                            ...
                            className={`transition-colors ${
                              tableDragOverIdx === rowIdx && tableDragIdx !== rowIdx ? 'bg-blue-500/10 border-t-2 border-t-blue-500' :
                              tableDragIdx === rowIdx ? 'opacity-40 bg-slate-700/20' :
                              isExpanded ? 'bg-slate-700/20' : 'hover:bg-slate-700/20'
                            }`}>
                            ...
                            <td className="px-4 py-4">
                              {item.changePct != null && item.change != null ? (
                                <div className={`inline-flex flex-col px-2 py-0.5 rounded-md font-mono text-xs tabular-nums font-bold ${
                                  item.changePct > 0 ? 'bg-emerald-500/15 border border-emerald-500/30 text-emerald-300' :
                                  item.changePct < 0 ? 'bg-rose-500/15 border border-rose-500/30 text-rose-300' :
                            ...
                            <td className="px-4 py-4">
                              {item.macdHistogram != null ? (() => {
                                ...
                                return <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md font-mono text-xs font-bold tabular-nums ${pos?'bg-emerald-500/15 border border-emerald-500/30 text-emerald-300':'bg-rose-500/15 border border-rose-500/30 text-rose-300'}`}>
      ...
      {/* Manage Modal */}
      <ManageModal
        show={wl.showManageModal}
        manageWatchlists={wl.manageWatchlists}
        setManageWatchlists={wl.setManageWatchlists}
        activeWatchlistId={wl.activeWatchlistId}
        setActiveWatchlistId={wl.setActiveWatchlistId}
        renamingId={wl.renamingId}
        setRenamingId={wl.setRenamingId}
        renamingValue={wl.renamingValue}
        setRenamingValue={wl.setRenamingValue}
        dragIndex={wl.dragIndex}
        dragOverIndex={wl.dragOverIndex}
        commitRename={wl.commitRename}
        saveManageModal={wl.saveManageModal}
        handleModalDragStart={wl.handleModalDragStart}
        handleModalDragOver={wl.handleModalDragOver}
        handleModalDrop={wl.handleModalDrop}
        handleModalDragEnd={wl.handleModalDragEnd}
      />
    </div>
  );
}
