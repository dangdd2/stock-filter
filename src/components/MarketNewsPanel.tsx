    ...
    <div className="space-y-3">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 flex flex-wrap items-center gap-3">
        ...
        <form onSubmit={handleSearch} className="flex gap-1.5 ml-auto">
          ...
          <button type="submit" className="px-3 py-1.5 bg-blue-500/15 text-blue-400 border border-blue-500/25 rounded-md text-xs hover:bg-blue-500/25 transition-colors">
            Tìm
          </button>
          {search && (
            <button type="button" onClick={() => { setSearch(''); setSearchInput(''); }} className="px-2 py-1.5 text-slate-400 hover:text-slate-200 text-xs">✕</button>
          )}
        </form>
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {SOURCES.map(s => {
            const count = s.key === 'all' ? articles.length : (sourceGroups[s.key] ?? 0);
            return (
        ...
        {articles.length > 0 && (
          <div className="divide-y divide-slate-700/30">
            {articles.map(article => (
              <div key={article.id}
                className="grid grid-cols-[140px_1fr_140px] gap-4 px-4 py-3 hover:bg-slate-700/20 transition-colors group">
                {/* Time */}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    {article.ticker && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-300 border border-amber-500/25 font-mono font-bold">
                        {article.ticker}
                      </span>
                    )}
      ...
      {articles.length > 0 && (
        <p className="text-center text-[11px] text-slate-600">{articles.length} tin tức • Cập nhật mỗi lần tải trang</p>
      )}
    </div>
  );
}
