...
interface Props {
  show: boolean; manageWatchlists: Watchlist[]; setManageWatchlists: (v: Watchlist[]) => void;
  activeWatchlistId: string; setActiveWatchlistId: (id: string) => void;
  ...
  commitRename: () => void; saveManageModal: () => void;
...
export default function ManageModal({ show, manageWatchlists, setManageWatchlists, activeWatchlistId, setActiveWatchlistId, renamingId, setRenamingId, renamingValue, setRenamingValue, dragIndex, dragOverIndex, commitRename, saveManageModal, handleModalDragStart, handleModalDragOver, handleModalDrop, handleModalDragEnd }: Props) {
  if (!show) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={e => { if (e.target === e.currentTarget) saveManageModal(); }}>
      <div className="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md mx-4 flex flex-col max-h-[80vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700 shrink-0">
          ...
          <button onClick={saveManageModal} className="text-slate-400 hover:text-slate-200 transition-colors"><X size={18}/></button>
        </div>
        <div className="px-5 py-3 overflow-y-auto flex-1">
          ...
          <ul className="space-y-1">
            {manageWatchlists.map((w, idx) => {
              ...
              return (
                <li key={w.id} draggable={!isMaster}
                  ...
                  {isRenaming ? (
                    ...
                    <span className={`flex-1 text-sm font-medium ${isMaster?'text-slate-400':'text-slate-200 hover:text-blue-300 cursor-text'}`}
        ...
        <div className="px-5 py-4 border-t border-slate-700 flex justify-end shrink-0">
          <button onClick={saveManageModal} className="flex items-center gap-1.5 px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs rounded-lg transition-colors font-medium">
            <Save size={13}/> Save
          </button>
        </div>
      </div>
